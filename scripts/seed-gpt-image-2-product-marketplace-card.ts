/**
 * GPT Image 2 — Product Marketplace Card (PRODUCT_CARD / PRODUCT_MARKETPLACE_CARD).
 * Идемпотентный upsert, dry-run, активация и назначение в AppSetting.
 * npm run seed:gpt-image-2-product-marketplace-card
 */
...
const SLUG = "gpt-image-2-product-marketplace-card";
...
const APP_SETTING_KEY = "PRODUCT_CARD_DEFAULT_MARKETPLACE_CARD_MODEL_SLUG";
...
const RAW_PAYLOAD_EXAMPLE = {
  ...
  input: {
    prompt:
      "Create a clean marketplace product card from the uploaded product photo. Keep the real product recognizable, preserve its shape, color and key details. Use a premium commercial marketplace style, clean background, readable composition, and space for short product benefits.",
...
function fail(msg: string): never {
  console.error(`[seed:gpt-image-2-product-marketplace-card] FAIL: ${msg}`);
  process.exit(1);
}
...
function validateModelData() {
  ...
  if (!API_MODEL_ID.trim()) fail("apiModelId empty");
  if (!ENDPOINT.trim()) fail("endpoint empty");
  if (!STATUS_ENDPOINT.trim()) fail("statusEndpoint empty");

  console.log("[seed:gpt-image-2-product-marketplace-card] model data OK");
  return parsed;
}
...
async function main() {
  ...
  const metadata = {
    ...
    marketplaceUrl: "https://kie.ai/gpt-image-2?model=gpt-image-2-image-to-image",
    docsCheckedAt: new Date().toISOString().slice(0, 10),
    source: "docs.kie.ai + kie.ai playground",
    purpose: "product_marketplace_card",
    ownerHint: "Use for product photo → marketplace card generation",
    ...
    kieNotes: {
      ...
      resolutionWarning:
        "1:1 cannot use 4K. auto or missing aspect_ratio should use only 1K.",
    },
  };

  const common = {
    name: "GPT Image 2 — Product Marketplace Card",
  ...
  console.log(
    `[seed:gpt-image-2-product-marketplace-card] upserted ${SLUG} id=${model.id} isActive=${model.isActive}`,
  );
  ...
  if (!dryRun.ok) {
    fail(`dry-run build failed: ${dryRun.error}`);
  }
  ...
  console.log("[seed:gpt-image-2-product-marketplace-card] dry-run payload:");
  console.log(JSON.stringify(dryRun.payload, null, 2));
  console.log(
    `[seed:gpt-image-2-product-marketplace-card] dry-run warnings: ${dryRun.warnings.join("; ") || "none"}`,
  );
  ...
  if (!model.isActive) {
    ...
    console.log("[seed:gpt-image-2-product-marketplace-card] activated model");
  }
  ...
  console.log(
    `[seed:gpt-image-2-product-marketplace-card] assigned ${APP_SETTING_KEY}=${SLUG}`,
  );

  if (nonCritical.length > 0) {
    console.log(
      `[seed:gpt-image-2-product-marketplace-card] non-critical warnings: ${nonCritical.join("; ")}`,
    );
  }

  console.log("[seed:gpt-image-2-product-marketplace-card] OK");
}
