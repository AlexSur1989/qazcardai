/**
 * Создаёт или обновляет видео-модель Bytedance Seedance 2.0 (Kie.ai).
 * Запуск: npm run seed:seedance
 */
...
const SETTINGS_SCHEMA = {
  fields: [
    ...
    },
    {
      ...
      type: "boolean",
      label: "Вернуть последний кадр (return_last_frame, в API отмечено как deprecated)",
      default: false,
      required: false,
    },
    {
      name: "generateAudio",
      type: "boolean",
      label: "Сгенерировать звук (generate_audio; в Kie API по умолчанию true, выше стоимость при включении)",
      default: false,
      required: false,
    },
    {
    ...
    },
    {
      name: "nsfwChecker",
      type: "boolean",
      label: "Kie-фильтр контента (nsfw_checker; false — фильтр отключён, как в доке по умолчанию)",
      default: false,
      required: false,
    },
  ],
...
async function main() {
  ...
  const row = await prisma.aiModel.upsert({
    ...
    create: {
      ...
      maxDuration: 15,
      description:
        "Bytedance Seedance 2.0 (Kie: bytedance/seedance-2, POST /api/v1/jobs/createTask). Сценарии text-to-video, first/last frame, reference (изображения/видео/аудио) взаимоисключающие; resolution 480p/720p/1080p, aspect_ratio включая adaptive, длительность 4–15 с. Док: docs.kie.ai/market/bytedance/seedance-2",
